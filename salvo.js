...
const updateVar = ((path, data, action, sourcePath) => {
  ...
  if (action === 'set_from_path') {
    //path in this context means the source to the data, not the target of the data

    storedValues[path] = typeof data === 'string' ? _.get(JSON.parse(data), sourcePath) : _.get(data, sourcePath);
  }
});
    ...
    .then(() => {
      return new Promise(actionPromise => {
        ...
        if (action.type === 'set-var') {
          updateVar(action.values.target, action.values.data, action.values.action, action.values.sourcePath);
          return actionPromise();
        }
        ...
        if (action.type === 'make-file') {
          return new Promise(resolve2 => {
            ...
            if (dataOperations.indexOf('splitbycommas') > -1) {
              writeData = (writeData).split(",").join("\r\n");
            }
            if (dataOperations.indexOf('jsonreadability') > -1) {
              writeData = (writeData).split("\"").join("").split("[").join("").split("]").join("");
            }
